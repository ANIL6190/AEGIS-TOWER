...
MODEL_DIR = "d:/ANIL A/Documents/AEGIS TOWER/backend"
...
def train_models():
    ...
    # 3. Collision Probability Regressor
    model_prob = RandomForestRegressor(n_estimators=100, random_state=42, max_depth=12)
    model_prob.fit(X_train, y_train_prob)
    pred_prob = model_prob.predict(X_test)
    mae_prob = mean_absolute_error(y_test_prob, pred_prob)
    r2_prob = r2_score(y_test_prob, pred_prob)
    print(f"Collision Probability Model - MAE: {mae_prob:.6f}, R2: {r2_prob:.4f}")
